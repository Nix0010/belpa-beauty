// ============================================================
// BELPA WEB — SUITE DE AUDITORÍA Y PRUEBAS AUTOMATIZADAS: BLOQUE 8
...
async function runTestSuite() {
  console.log('\n============================================================');
  console.log('🌸 BELPA WEB — AUDITORÍA AUTOMATIZADA BLOQUE 8');
  console.log('============================================================\n');
  ...
  const block8SqlPath = path.join(supabaseDir, 'block8_database.sql');

  let sectionStatus = {
    auditoriaPrevia: true,
    supabaseConfig: 'PENDIENTE',
    authentication: 'PENDIENTE',
    rls: 'PENDIENTE',
    productsCloud: 'PENDIENTE',
    ordersCloud: 'PENDIENTE',
    orderItems: 'PENDIENTE',
    statusHistory: 'PENDIENTE',
    storage: 'PENDIENTE',
    sincronizacion: true,
    checkout: true,
    whatsapp: true,
  ...
  try {
    ...
    assert(adminJs.includes('syncPendingOrders'), 'admin.js implementa syncPendingOrders() para sincronizar pedidos con Supabase Cloud');
  } catch (err) {
  ...
  try {
    ...
    assert(adminHtml.includes('order-quick-pill'), 'HTML contiene grupo de pills rápidas de estado');
  ...
  // ------------------------------------------------------------
  // 7. ESQUEMA SQL UNIFICADO (block8_database.sql)
  // ------------------------------------------------------------
  console.log('\n--- 7. Esquema SQL Unificado (block8_database.sql) ---');
  try {
    assert(fs.existsSync(block8SqlPath), 'supabase/block8_database.sql existe en disco');
    const sql = fs.readFileSync(block8SqlPath, 'utf8');
    ...
    assert(sql.includes('get_next_order_number()'), 'SQL define función atómica get_next_order_number()');
    assert(sql.includes('is_admin()'), 'SQL define función de verificación is_admin()');
  ...
  // ------------------------------------------------------------
  // 8. INDICADOR DE CONEXIÓN Y ESTADOS
  // ------------------------------------------------------------
  console.log('\n--- 8. Máquina de Estados del Indicador de Conexión ---');
  try {
    const adminJs = fs.readFileSync(adminJsPath, 'utf8');
    assert(adminJs.includes('updateConnectionStatus'), 'admin.js implementa updateConnectionStatus()');
    assert(adminJs.includes('⚪ Modo Local (Fallback)'), 'admin.js implementa estado ⚪ Local / Fallback');
    assert(adminJs.includes('🟢 Supabase Conectado'), 'admin.js implementa estado 🟢 Conectado');
    assert(adminJs.includes('🟡 Cloud (Sin respuesta)'), 'admin.js implementa estado 🟡 Warning');
  } catch (err) {
    assert(false, 'Indicador de Conexión', err.message);
  }

  // ------------------------------------------------------------
  // 9. AUDITORÍA DE SEGURIDAD Y PRIVACIDAD
  // ------------------------------------------------------------
  console.log('\n--- 9. Auditoría de Seguridad & Detección de Secretos ---');
  try {
  ...
  try {
    ...
    sectionStatus.http = false;
    assert(false, 'Verificación HTTP', err.message);
  }

  // ------------------------------------------------------------
  // 11. REPORTE FINAL ESTRUCTURADO BLOQUE 8 (SECCIÓN 30)
  // ------------------------------------------------------------
  console.log('\n============================================================');
  console.log('🌸 BELPA WEB — AUDITORÍA BLOQUE 8');
  console.log('============================================================\n');

  console.log(`Auditoría previa:\n  ${sectionStatus.auditoriaPrevia ? 'PASS' : 'FAIL'}\n`);
  console.log(`Supabase Configuration:\n  ${sectionStatus.supabaseConfig}\n`);
  console.log(`Authentication:\n  ${sectionStatus.authentication}\n`);
  console.log(`RLS:\n  ${sectionStatus.rls}\n`);
  console.log(`Products Cloud:\n  ${sectionStatus.productsCloud}\n`);
  console.log(`Orders Cloud:\n  ${sectionStatus.ordersCloud}\n`);
  console.log(`Order Items:\n  ${sectionStatus.orderItems}\n`);
  console.log(`Status History:\n  ${sectionStatus.statusHistory}\n`);
  console.log(`Storage:\n  ${sectionStatus.storage}\n`);
  ...
  console.log(`HTTP:\n  ${sectionStatus.http ? 'PASS' : 'FAIL'}\n`);
  console.log(`Tests Locales:\n  ${passedTests} / ${totalTests}\n`);
  console.log('Tests Cloud:\n  PENDIENTE\n');

  console.log('============================================================');
  console.log('RESULTADO BLOQUE 8:');
  console.log(failedTests === 0 ? '🌸 BLOQUE 8 — PASS LOCAL / CLOUD PENDIENTE' : 'FAIL');
  console.log('============================================================\n');
